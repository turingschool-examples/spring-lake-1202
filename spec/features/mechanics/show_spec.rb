...
RSpec.describe "Mechanics Show Page", type: :feature do
  before(:each) do 
    ...
    @mechanics_rides_1 = MechanicsRide.create!(mechanic_id: @mechanic_1.id, ride_id: @ride_2.id)
  end
  describe "When a user visits the mechanic show page" do
    it "displays mechanic's name, years of experience, and names of all the rides they are working on" do
      ...
      expect(page).to_have_content(@mechanic_1.name)
      expect(page).to_have_content(@mechanic_1.years_experience)
      expect(page).to_have_content(@ride_1.name)
      expect(page).to_have_content(@ride_2.name)
    end
